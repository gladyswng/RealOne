...
export const VALIDATOR_MATCH = (val: string) => ({ type: VALIDATOR_TYPE_MATCH, val: val })




export const validate = (value: string , validators: {type: string, val?: string | number  }[] ) : boolean => {
  let isValid = true
  for (const validator of validators) {
    ...
    if (validator.val && validator.type === VALIDATOR_TYPE_MINLENGTH) {
      isValid = isValid && value.trim().length >= validator.val
    }
    if (validator.val && validator.type === VALIDATOR_TYPE_MAXLENGTH) {
      isValid = isValid && value.trim().length <= validator.val
    }
